				...
				<div class="row">
					<div class="col-sm-3">
						<div class="form-group">
							<select class="custom-select" onchange="form.submit();" name="co_name">
								<option value="">==All Company==</option>
								<?php 
									$get_company = $db->query("SELECT * FROM tbl_company ORDER BY co_name ASC");
									while ($row_company = mysqli_fetch_object($get_company)) {

										echo '<option value="'.$row_company->co_id.'" '.(($row_company->co_id==@$_GET['co_name'])?'selected':'').'>'.$row_company->co_name.'</option>';

										// if($row_company->co_id == @$_GET['co_name']){
										// 	echo '<option SELECTED value="'.$row_company->co_id.'">'.$row_company->co_name.'</option>';
											
										// }else{
										// 	echo '<option value="'.$row_company->co_id.'">'.$row_company->co_name.'</option>';
										// }
									}
								?>
							</select>
					...
					<div class="col-sm-3">
						<div class="form-group">
							<select class="custom-select" onchange="form.submit();" name="s_name">
								...
								<?php 
									...
									while ($row_service = mysqli_fetch_object($get_service)) {
										...
										// if($row_service->s_id == @$_GET['s_name']){
										// 	echo '<option SELECTED value="'.$row_service->s_id.'">'.$row_service->s_name.'</option>';
											
										// }else{
										// 	echo '<option value="'.$row_service->s_id.'">'.$row_service->s_name.'</option>';
										// }
									}
								?>
							</select>
					...
					<div class="col-sm-3">
						<div class="form-group">
							<select class="custom-select" onchange="form.submit();" name="em_name">
								...
								<?php 
									...
									while ($row_employee = mysqli_fetch_object($get_employee)) {
										...
										// if($row_employee->em_id == @$_GET['em_name']){
										// 	echo '<option SELECTED value="'.$row_employee->em_id.'">'.$row_employee->em_name.'</option>';
											
										// }else{
										// 	echo '<option value="'.$row_employee->em_id.'">'.$row_employee->em_name.'</option>';
										// }
									}
								?>
							</select>
					...
					<div class="col-sm-3">
						<div class="form-group">
							<select class="custom-select" onchange="form.submit();" name="td_status" id="td_status">
